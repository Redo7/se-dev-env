...
import { DropdownMenu, DropdownMenuContent, DropdownMenuRadioItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger, DropdownMenuRadioGroup, } from "@/components/ui/dropdown-menu"
	  ...
	  return overlays
	    .filter(
	      (overlay) =>
	        !overlay.deleteAfter &&
	        (filterInput === "" ||
	          overlay.name.toLowerCase().includes(filterInput))
	    )
	    .sort(sortOverlays);
			...
			<div className="home-screen flex p-50 w-full">
				...
				<div className="recent-overlays flex flex-col gap-1.5 w-100 box-content">
					...
					<div className="overlay-container overflow-hidden flex flex-col gap-4">
						{recentOverlays.map((overlay: OverlayInstance) => { return ( <HomeScreenOverlay key={overlay.id} overlay={overlay} onOverlayDelete={softRemoveOverlay} /> ); })}
					</div>
				</div>
				{/* All Overlays */}
				<div className="all-overlays flex flex-col gap-1.5 grow">
					<div className='min-h-10 w-full pl-10 flex justify-between items-center align-middle'>
						<span className='flex gap-2 items-center'>
							...
							<DropdownMenu>
								<DropdownMenuTrigger className='hover:bg-tr-100 p-2 rounded-sm transition-colors flex items-center gap-1'>{sortOrder.includes("A") ? <BookA size={16} strokeWidth={1.5} /> : <ClockFading size={16} strokeWidth={1.5} />}<p className='text-sm'>{sortOrder}</p></DropdownMenuTrigger>
								<DropdownMenuContent>
									<DropdownMenuRadioGroup value={sortOrder} onValueChange={setSortOrder}>
										<DropdownMenuLabel className='text-[0.75rem] opacity-50 px-1.5 flex gap-1 items-center'><BookA size={12} strokeWidth={1.5} /> Alphabetical</DropdownMenuLabel>
										<DropdownMenuRadioItem value='A-Z'>A-Z</DropdownMenuRadioItem>
										<DropdownMenuRadioItem value='Z-A'>Z-A</DropdownMenuRadioItem>
										<DropdownMenuSeparator />
										<DropdownMenuLabel className='text-[0.75rem] opacity-50 px-1.5 flex gap-1 items-center'><ClockFading size={12} strokeWidth={1.5}/> Date</DropdownMenuLabel>
						...
						<Input onChange={handleFilterInput} className='w-60 border-0 border-b rounded-none bg-transparent dark:bg-transparent! focus-visible:ring-[0px]' placeholder='Search' />
					</div>
					<div className="flex gap-4 items-center 2-full pl-10">
						<p>All Overlays</p>
						<Separator className="data-[orientation=horizontal]:w-auto grow" />
					</div>
					<div ref={containerRef} className="all-overlays-container flex flex-col gap-4 flex-wrap h-full overflow-scroll pl-10 pr-4 scroll-px-10 snap-x snap-mandatory">
						{visibleOverlays.map((overlay: OverlayInstance) => { return ( <HomeScreenOverlay key={overlay.id} overlay={overlay} onOverlayDelete={softRemoveOverlay} /> ); })}
					</div>
				</div>
			</div>
		</div>
