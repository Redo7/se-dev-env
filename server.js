...
const SCRIPT_VER = 1.4; // Latest version of the iframe. Used to track whether any changes were made to the templates/iframe/ files, which would require widgets to have those files copied into their dir in order to work again.
...
app.get('/api/get-overlays', async (req, res) => {
	try {
		...
		const overlays = dirents.filter((dirent) => dirent.isDirectory()).map((dirent) => dirent.name);
...
app.put('/api/soft-delete/', async (req, res) => {
	...
	try {
		...
		if (!deletionData[overlayID])
			deletionData[overlayID] = {
				name: overlayName,
				id: overlayID,
				widgets: [],
			};
		...
		if (widgetName && widgetID) {
			...
			overlayData.widgets = overlayData.widgets.map((widget) =>
				widget.id === widgetID ? { ...widget, deleteAfter: deletionDate } : widget,
			);
	...
	try {
		if (widgetID) {
			...
			const updatedWidgetsArray = {
				...currWidgetsArray,
				widgets: currWidgetsArray.widgets.filter((widget) => widget.id != widgetID),
			};
		...
		if (Object.keys(deletionData[overlayID].widgets).length <= 1) {
			...
			const updatedDeletionDataWidgets = deletionData[overlayID].widgets.filter(
...
app.post('/api/field-data/:overlay/:widget', async (req, res) => {
	...
	try {
		...
		let content, hasNewFields = false;
...
app.put('/api/update-iframe-files/', async (req, res) => {
	...
	copyFiles(iframeTemplate, widgetPath, ['iframe.html', 'main.css', 'script.js'])
		.then(async () => {
...
app.post('/api/widget-io-import/:overlayID', upload.single('file'), async (req, res) => {
	...
	try {
		...
		for (const [section, outFile] of Object.entries(mapping)) {
			...
			const zipEntry = entries.find((e) => basename(e.entryName) === basename(entryPath));
...
app.post('/api/rename/', async (req, res) => {
	...
	try {
		...
		if (widgetID != undefined) {
			const updatedData = overlayData.widgets.map((widget) =>
				widget.id === widgetID
					? { ...widget, name, id: newID, src: widget.src.replace(widgetID, newID) }
					: widget,
			);
			...
			const updatedWidgetData = overlayData.widgets.map((widget) => ({
				...widget,
				src: widget.src.replace(overlayID, newID),
			}));
...
app.get('/api/open-folder/:overlayID/:widgetID', async (req, res) => {
	...
	const instancePath =
		widgetID == 'null' ? join(__dirname, 'overlays', overlayID) : join(__dirname, 'overlays', overlayID, widgetID);
...
app.put('/api/copy-to/', async (req, res) => {
	try {
		...
		targetOverlayData.widgets = targetOverlayData.widgets.map((widget) =>
			widget.id === widgetID
				? { ...widget, id: newID, src: widget.src.replace(overlayID, targetOverlay).replace(widgetID, newID) }
				: widget,
		);
